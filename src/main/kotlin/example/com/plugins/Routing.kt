    ...
    routing {
        ...
        patch("/users/{email}/details") {
            try {
                ...
                val newCpf = call.request.queryParameters["newCpf"]
                val newImage = call.request.queryParameters["newImage"]

                if (email == null || newName == null || newCpf == null || newImage == null) {
                    call.respond(HttpStatusCode.BadRequest, "Email, imagem ou telefone inválidos!")
                    return@patch
                }

                if (userRepository.updateUserDetails(email, newName, newCpf, newImage)) {
                    call.respondText("Editado com sucesso", status = HttpStatusCode.OK)
                } else {
                    call.respondText("Erro", status = HttpStatusCode.BadRequest)
                }
            } catch (e: Exception) {
                call.respondText("Erro editando o seguinte status: $e", status = HttpStatusCode.BadRequest)
            }
        }
